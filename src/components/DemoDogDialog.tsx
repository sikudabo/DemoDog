    ...
    return (
        ...
        >
            <DialogTitle style={{ display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                {isError ? <p style={{ color: colors.error, fontWeight: 900 }}>{title}</p> : ''}
            </DialogTitle>
            <DialogContent>
                <DialogContentText>
                    {message}
            ...
            <DialogActions>
                <DemoDogButton buttonColor={isError ? colors.error : colors.navyBlue} onClick={handleCloseDialog} text="OK" isTextBtn />
            </DialogActions>
        </Dialog>
    );  
}
